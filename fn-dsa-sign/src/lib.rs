...
pub trait SigningKey: Sized {
    ...
    ///
    /// The provided `seed` is expanded with SHAKE256 to provide all the
    /// randomness used by the signing process (including the nonce and
    /// Gaussian sampling).
    fn sign_seeded(&mut self, seed: &[u8],
        ctx: &DomainContext, id: &HashIdentifier, hv: &[u8], sig: &mut [u8])
    {
        let mut rng = SeededRng::new(seed);
        self.sign(&mut rng, ctx, id, hv, sig);
    }

    /// Create a sampler for use with `sign_attempt`.
    fn create_sampler(&self, seed: &[u8; 56]) -> Sampler;

    /// Perform one signing attempt with a pre-computed hash-to-point.
    ///
    /// This is a low-level building block for custom signing flows that
    /// use a different hash-to-point function (e.g., RPO256 instead of
    /// SHAKE256).
    ///
    /// Returns `true` if the signature was successfully generated, `false`
    /// if the attempt failed. On failure, the sampler state has evolved,
    /// so calling again will produce different samples.
    ///
    /// Parameters:
    ///  - `hm`: hash-to-point result (polynomial coefficients mod q)
    ///  - `nonce`: 40-byte nonce to include in signature header
    ///  - `sampler`: sampler created with `create_sampler`
    ///  - `sig`: output buffer for signature
    fn sign_attempt(&mut self, hm: &[u16], nonce: &[u8; 40],
        sampler: &mut Sampler, sig: &mut [u8]) -> bool;
}

// ============================================================================
// Sampler (opaque wrapper)
// ============================================================================

/// Opaque sampler for Gaussian sampling during signing.
///
/// Create with [`SigningKey::create_sampler`], use with [`SigningKey::sign_attempt`].
/// The sampler state evolves with each signing attempt, enabling retries to
/// produce different samples.
pub struct Sampler {
    inner: sampler::Sampler<shake::SHAKE256_PRNG>,
}

impl Sampler {
    fn new(logn: u32, seed: &[u8; 56]) -> Self {
        Self {
            inner: sampler::Sampler::new(logn, seed),
        }
    }
}

// ============================================================================
// SeededRng for deterministic signing
// ============================================================================

struct SeededRng(shake::SHAKE256);
    ...
    ($typename:ident, $logn_min:expr, $logn_max:expr) =>
{
    ...
    impl SigningKey for $typename {
        ...
        {
            let n = 1usize << self.logn;
            sign_attempt_inner(self.logn, hm, nonce, &mut sampler.inner, sig,
                #[cfg(not(feature = "small_context"))]
                &self.basis[..(4 * n)],
                &self.f[..n], &self.g[..n], &self.F[..n], &self.G[..n],
                &mut self.tmp_i16, &mut self.tmp_flr)
        }
    }
} }
...
/// Core signing attempt - returns true on success, false if retry needed.
/// The sampler state evolves on each call, enabling retries with different samples.
#[allow(unused_variables)]
fn sign_attempt_inner<P: PRNG>(logn: u32, hm: &[u16], nonce: &[u8; 40],
    samp: &mut sampler::Sampler<P>, sig: &mut [u8],
    #[cfg(not(feature = "small_context"))]
    basis: &[flr::FLR],
    f: &[i8], g: &[i8], F: &[i8], G: &[i8],
    tmp_i16: &mut [i16], tmp_flr: &mut [flr::FLR]) -> bool
{
    let n = 1usize << logn;
    ...
    // need to return false in these cases.
    ...
    tmp_i16: &mut [i16], tmp_u16: &mut [u16], tmp_flr: &mut [flr::FLR])
{
    ...
    loop {
        ...
        // In the original Falcon, hash-to-point was done once; in FN-DSA,
        // it is done at each loop restart.
        ...
        if sign_attempt_inner::<P>(logn, hm, &nonce, &mut samp, sig,
            #[cfg(not(feature = "small_context"))]
            basis,
            f, g, F, G, tmp_i16, tmp_flr)
        {
            return;
        }
    }
